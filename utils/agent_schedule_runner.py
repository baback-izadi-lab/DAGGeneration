...
class ScheduleRunner:
    def __init__(self, schedule=None,
        ...
        for task_num, task in self.all_tasks.items():
            parents = set(self.schedule[task_num]['parents'])
            children = set()
            for child_num in self.schedule[task_num]['children']:
    ...
    @property
    def task_energies(self):
        energies = []
        for proc in self.processor_times:
            energies.append(sum([task[3] for task in proc]))
        return energies
