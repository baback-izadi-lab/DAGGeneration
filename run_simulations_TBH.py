...
def run(beta, all_speeds, base_path, sim_data, algorithm_name,
        dls_algo=False, base_power_min=False, agent_system=True):
    """
    ...
    for processor_speeds in all_processors_speeds:
        ...
        if processor_speeds == (0, 0, 1, None, None) and beta == 1.0:
            print(base_powers)
            print(agent_system)
            print(beta)
...
def run_all(total_tasks, algorithm_name, ):
    ...
    parser.parse(base_path+f'DAG_{total_tasks}_5.tgff',
...
if __name__ == '__main__':
    ...
    for tasks in total_tasks:
        run_all(tasks, 'TBH')
        print(f'TBH for DAG {tasks} complete')
